...
async fn handle_action(
    ...
    match action {
        ...
        Action::FirmwareUpgrade => {
            handle_firmware_upgrade_action(config, controller_response.data).await?;
        }
    }
    Ok(())
}
...
async fn handle_firmware_upgrade_action(
    ...
    let http_client = client::create_http_client(config)?;
    ...
    // TODO: Report upgrade status back to carbide-api via ReportScoutFirmwareUpgradeStatus RPC.
