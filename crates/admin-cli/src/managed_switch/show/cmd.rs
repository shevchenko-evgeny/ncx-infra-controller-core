...
use rpc::forge::{LinkedExpectedSwitch, Switch};
use serde::Serialize;
...
fn show_table_view(outputs: &[ManagedSwitchOutput]) {
    println!(
        "{:<36} {:<20} {:<18} {:<18} {:<10} {:<10} {:<15}",
        "Switch ID", "Name", "Serial", "BMC MAC", "Power", "Health", "State"
    );
    println!("{:-<140}", "");
...
fn show_csv(outputs: &[ManagedSwitchOutput]) {
    println!(
        "Switch ID,Name,Serial Number,BMC MAC,BMC IP,Power State,Health,State,Expected Switch ID,Rack ID"
    );
    for m in outputs {
        println!(
            "{},{},{},{},{},{},{},{},{},{}",
...
pub async fn handle_show(
    ...
    let outputs = build_managed_switch_outputs(switches, linked);
